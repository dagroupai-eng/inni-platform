...
## 출처 인용 규칙 (Citation Rules) - 반드시 준수
- **업로드 문서**: `[파일명.pdf p.N]` 형식 — 해당 수치·문장이 제공된 문서에 실제로 존재할 때만 사용
- **법령/통계**: "건축법 제XX조", "국토교통부 2024년 보고서" 형식으로 기관·연도 명시
- **AI 추론**: 문서에 없는 수치·사실은 반드시 **(AI 추론)** 마커 표기
  - 수치: `(AI 추론 | 근거: [출처] | 적용: [산식] | 범위: [통상 범위])`
  - 정성 판단: `(AI 추론 | 근거: [판단 근거])` 표기 — 근거 없이 (AI 추론) 단독 사용 금지
- **모든 표에 '근거·출처' 열 포함** (❌ 빈 셀 금지)
- ❌ 절대 금지: 문서에 없는 내용에 `[파일명.pdf p.N]` 표기 / `[출처1]` 등 번호만 표기

## 표 작성 규칙 (Table Formatting Rules) - 반드시 준수
1. **같은 내용을 절대 반복하지 마세요.** 한 번 작성한 섹션이나 문단은 다시 작성하지 않습니다.
2. **표는 반드시 완전한 마크다운 형식으로 작성하세요:**
   - 모든 행은 `|`로 시작하고 `|`로 끝나야 합니다
   - 헤더 행 다음에는 반드시 구분선 `|---|---|`이 있어야 합니다
   - 모든 셀에 내용을 채워야 합니다 (빈 셀은 `-` 또는 `N/A`로 표시)
3. **올바른 표 예시:**
| 항목 | 내용 |
|------|------|
| 프로젝트명 | 삼척 스포츠 아카데미 |
...
5. 표가 잘리거나 불완전하면 차라리 글머리 기호(bullet point)로 작성하세요.
