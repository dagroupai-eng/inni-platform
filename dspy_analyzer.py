...
### 3. 문서에 없는 내용: (AI 추론) 마커 필수
- **문서에 명시되지 않은 수치·사실을 분석에 활용할 경우, 반드시 (AI 추론) 마커를 붙이세요**
  - 수치: `(AI 추론 | 근거: [출처] | 적용: [산식] | 범위: [통상 범위])`
  - 정성 판단: `(AI 추론 | 근거: [판단 근거])` — 근거 없이 (AI 추론) 단독 사용 금지
- **문서에서 전혀 확인되지 않고, 추론도 불가한 정보는 '문서에 명시되지 않음' 또는 '추가 확인 필요'로 표시하세요**
- **❌ 금지: 문서에 없는 내용을 [파일명.pdf p.N] 출처로 표기하거나, 마커 없이 사실인 것처럼 서술하는 것**
...
"""
    ...
    def _build_system_instruction(self, block_info: Dict[str, Any]) -> str:
        ...
        system_parts = [f"당신은 {role}입니다."]

        if instructions:
            system_parts.append(instructions)

        if end_goal:
            system_parts.append(f"## 최종 목표\n{end_goal}")

        system_parts.append(
            "## 핵심 분석 원칙\n"
            "- 모든 분석은 제공된 데이터에 근거하며, 객관성을 유지합니다.\n"
            "- 유리·불리·조건부를 데이터에 따라 솔직하게 제시합니다 (근거 없는 긍정 결론 금지).\n"
            "- 상세하고 풍부한 내용으로 작성합니다 (표, 수치, 사례 포함, 최소 1000자 이상).\n"
            "- 각 블록의 고유한 분석만 수행합니다 (이전 블록 내용 반복 금지)."
        )

        system_parts.append(
            "## 출처 인용 규칙 (필수)\n"
            "- **업로드 문서**: [파일명.pdf p.N] 형식 — ⚠️ 반드시 제공된 Search Result 블록에 실제로 존재하는 수치·문장일 때만 사용. 블록 제목(예: 📌 파일명.pdf p.3)에서 파일명과 페이지번호만 추출해 [파일명.pdf p.N] 형식으로 인용\n"
            "- **웹 검색**: [기사 제목](URL) 형식\n"
            "- **법령/통계**: \"건축법 제XX조\", \"국토교통부 2024년 보고서\" (기관·연도 명시)\n"
            "- **AI 추론**: 제공 자료에 없는 수치·사실은 반드시 **(AI 추론)** 마커를 붙여 표기\n"
            "  · 수치가 있을 때: (AI 추론 | 근거: [출처] | 적용: [이 프로젝트 산식] | 범위: [통상 범위])\n"
            "  · 정성 판단·등급 정의 등 수치 없을 때: (AI 추론 | 근거: [판단 근거]) — 근거 없이 (AI 추론) 단독 사용 금지\n"
            "  예) Prime 임대료: 5~7만원/㎡ (AI 추론 | 근거: 한국감정원 2023 상업용부동산 | 적용: 강남 도심 Prime 기준 | 범위: 입지·층에 따라 4~8만원)\n"
            "  예) 오피스 A급 기준 — 임대면적 전용률 75~80% (AI 추론 | 근거: 업계 통상 기준, 한국부동산원 오피스 등급 분류)\n"
            "- ⚠️ **(AI 추론)은 citation과 독립적**: Search Result 블록에 '관련 내용'이 있어도, 해당 수치·기준이 그 블록에 **직접 명시**되어 있지 않으면 (AI 추론) 마커를 반드시 사용. '관련 블록이 존재한다'는 이유로 (AI 추론) 생략 불가.\n"
            "- ❌ 절대 금지: 제공된 블록에 해당 내용이 없는데 [파일명.pdf p.N] 출처 표기하는 것\n"
            "- ❌ 금지: [출처1], [웹검색1] 같은 번호만 표기\n"
            "- ❌ 금지: [참고 자료 index=\"N\"] 같은 인덱스 속성 형식 — 반드시 실제 파일명으로 인용\n"
            "- 모든 표에 '근거·출처' 열 포함\n"
            "- 웹 검색 도구 사용 시 분석 끝에 '## 📚 웹 검색 출처 목록' 섹션 추가 (실제 URL 기재)"
        )

        system_parts.append(
            "## 출력 형식\n"
            "- 마크다운 필수: 헤더(##/###), 표(파이프 형식), 볼드, 리스트\n"
            "- 표 셀은 한 줄로 작성 (줄바꿈·파이프(|) 금지), 헤더+구분선(---) 필수\n"
            "- 표 다음에는 반드시 핵심 내용·시사점을 7-8문장 줄글로 해설 (표만 단독으로 끝내지 말 것)\n"
            "- 직접 인용 시 \"제공된 자료에 따르면, '[원문 그대로]'\" 형식 사용"
        )
